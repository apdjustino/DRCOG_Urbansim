...
class Test_(OpusTestCase):
    ...
    def test_tag_name_fix(self):
        ...
        self.assert_(self.root.find('new_tag') is not None)
    ...
    def test_action_update_version(self):
        ...
        self.assert_(version_node.text == '2.0')
        # create the version if it doesn't exist
        self.root.remove(version_node)
        self.assert_(self.root.find('xml_version') is None)
    ...
    def test_fix_submodel_node(self):
        ...
        self.assert_(submodel_node.find('submodel_id') is None)
        self.assert_(submodel_node.get('submodel_id') == '-2')

    def test_move_node(self):
        node_to_move = self.root.find('general/project_name')
        path_to_move_to = 'model_manager/model_system'
        self.assert_(self.root.find('%s/%s' %(path_to_move_to, 'project_name')) is None)
        self.instance.action_move_node(node_to_move, path_to_move_to)
        self.assert_(self.root.find('%s/%s' %(path_to_move_to, 'project_name')) is node_to_move)
        node_to_move_to = self.root.find('results_manager')
        self.instance.action_move_node(node_to_move, node_to_move_to)
        self.assert_(node_to_move_to.find('project_name') is node_to_move)
        # move using an extended path
        self.instance.action_move_node(node_to_move, node_to_move_to, 'Simulation_runs')
        self.assert_(node_to_move_to.find('Simulation_runs/project_name') is node_to_move)
    ...
    def test_convert_selectable_list_node(self):
        ...
        for selectable_node in node:
            ...
            self.assert_(selectable_node.tag == selectable_node.get('type') == 'selectable')
            self.assert_(selectable_node.text in ['True', 'False'])
            self.assert_(selectable_node.get('name') == tag_before)
