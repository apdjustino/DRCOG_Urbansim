from bottle import route, response, run, hook, static_file, request
from synthicity.utils import yamlio
import simplejson
import numpy as np
import pandas as pd
import os
...
def queryData():
    ...
    queryParam = simplejson.loads(input)
    year = queryParam['year']
    field = queryParam['field_name']
    aggregation = queryParam['aggregation']


    df = DFRAMES['zone_data']

    results = df.loc[df.sim_year == year]
    cmd = "results.groupby('zone_id')['%s'].%s" % (field, aggregation)
    print cmd
    output = eval(cmd)
    #output = yamlio.series_to_yaml_safe(output.dropna())
    output = output.dropna().to_json()
    return output
...
def index():
    ...
    index = open(os.path.join(dir, 'dframe_explorer_new.html')).read()
