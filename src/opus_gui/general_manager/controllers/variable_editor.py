...
from PyQt4.QtGui import QDialog, qApp, QMessageBox
...
class VariableEditor(QDialog, Ui_VariableEditor):
    ...
    #def start_service(self,sqlitefile,tabledict,port,host='localhost',srid=3740,sqlite=1):
    def start_service(self,sqlitefile,tabledict):
        port = 9876
        host = 'localhost'
        srid = 3857
        sqlite = 0
        ...
        import random
        import sys, math, shutil, time, string
        AVAILABLEYEARS = [2010]
        ...
        def compute_variable(table,variable,xmin,ymin,xmax,ymax,srid=None,minmaxonly=0,sampleprop=None,qcut=0,bins=0):
            ...
            print table, variable, xmin, ymin, xmax, ymax
            ...
            if not minmaxonly:
              ...
              if SQLITE:
                ...
                print srid, xmin, xmax, ymin, ymax
                print time.ctime()
              ...
              subset = table
              if sampleprop and len(table.index) > 1200000:
            ...
            #exec("var=%s"%variable,globals(),ldict)  in globals(), locals()
            exec "var=%s"%variable in globals(), locals()
            vector = ldict['var']
        ...
        global CURSOR, TABLEDICT, TGTSRID, SQLITE, PARCELXYS
